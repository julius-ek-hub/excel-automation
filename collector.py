...
from utils import _dir_, sub_process, convert_bytes, _input_, resource_path, cprint, from_json_file
...
class Collector:
    ...
    def ask(self, question, answer_is_ok):
        ...
        if answer_is_ok(q):
            return q
        cprint('Sorry, I do not understand your choice!', lable=False, type='warning')
        return self.ask(question, answer_is_ok)

    def get_path_to_open(self, name: str, sufix="", important=True):

        path = _input_('\nFull path to ' + name + ' or hit Enter with no input to open file dialog' + sufix + ': ')
        path = path.replace('"', '').replace('\'', '').replace('\\', '/').strip()

        if not important and path.lower() == '--d':
            cprint('Default/Old --> OK', 'success')
            return None
        ...
        if not sheet_path_open_is_ok(path):
            ...
            return self.get_path_to_open(name, sufix, important)
        else:
            size = os.stat(path).st_size
            if size/1024/1024 > 2:
                cprint('File size larger than 2MB, Reduce file size by checking and removing excessive blanc rows' + ((' and/or convert your file to .xlsx manually (might reduce the size)' if not path.endswith('.xlsx') else '') + ' otherwise the scanning might take a while.'), 'warning')
    ...
    def collect_scans(self):
        ...
        if vulnerability_param == 'Internal':
            entity = self.get_text_from_options(from_json_file('./assets/entities.json'), 'Entity')
